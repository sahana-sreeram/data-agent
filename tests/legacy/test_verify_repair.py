...
from src.legacy.verify_repair import run_verify_repair
...
def _setup(tmp_path: Path):
    ...
    manifest = {
        ...
        "test_inventory": ["tests/legacy/test_transform.py"],
        "rerun": {
...
def test_repair_that_changes_raw_data_is_blocked(tmp_path, monkeypatch):
    ...
    import src.legacy.verify_repair as verify_repair_module
...
# patched src/legacy/transform.py from an isolated workspace and validating with
...
# compute_portfolio_summary_with_payment_join in src/legacy/transform.py).
...
def test_incorrect_join_code_change_repair_is_verified_and_promoted(tmp_path, monkeypatch):
    ...
    workspace_dir = Path(tempfile.mkdtemp())
    patched_transform_path = workspace_dir / "src" / "legacy" / "transform.py"
    ...
    repair_result = {
        ...
        "target_file": "src/legacy/transform.py",
        "changed_files": ["src/legacy/transform.py"],
        "workspace_dir": str(workspace_dir),
    }

    # Redirect ONLY the final promotion copy so the real repo's src/legacy/transform.py is never
    # touched, without changing cwd (test_inventory's nested pytest run needs the real cwd --
    # tests/legacy/test_transform.py's own CLI tests default to a cwd-relative business-rules path).
    import src.legacy.verify_repair as verify_repair_module
    ...
    def _redirect_copy2(src, dst, *args, **kwargs):
        if Path(dst) == Path("src/legacy/transform.py"):
