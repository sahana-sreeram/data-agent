...
from src.answer_models import AnswerStatus
from src.ask import AskError, answer_question
from src.business_agent import SUBMIT_ANSWER_TOOL_NAME
from src.diagnose_incident import DiagnoseIncidentError
...
def test_pass_validation_answers_directly_without_touching_diagnose_or_repair(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.ask.run_diagnose_incident", lambda *a, **k: (_ for _ in ()).throw(AssertionError("should not diagnose")))
    monkeypatch.setattr("src.ask.run_self_healing", lambda *a, **k: (_ for _ in ()).throw(AssertionError("should not self-heal")))
...
def test_fail_validation_heals_successfully_then_answers(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.ask.run_diagnose_incident", fake_diagnose)
    monkeypatch.setattr("src.ask.run_self_healing", fake_self_heal)
...
def test_diagnose_incident_error_degrades_to_unreliable_data(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.ask.run_diagnose_incident", fake_diagnose)
    monkeypatch.setattr("src.ask.run_self_healing", lambda *a, **k: (_ for _ in ()).throw(AssertionError("should not reach repair")))
...
def test_no_subprocess_usage_in_ask_module():
    tree = ast.parse(Path("src/ask.py").read_text())
