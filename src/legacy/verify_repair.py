...
def _load_patched_transform_module(workspace_dir: Path):
    """Dynamically import the patched copy of src/legacy/transform.py as an ISOLATED module object.

    Never touches sys.modules['src.legacy.transform'] -- the real, installed module
    is completely unaffected. This is how a CODE_CHANGE's patched behavior
    gets genuinely exercised before promotion.
    """
    patched_path = workspace_dir / "src" / "legacy" / "transform.py"
...
def _rerun_one_row_per_payment(manifest: dict, workspace_dir: Path, repair_result: dict) -> tuple[dict, dict]:
    from src.legacy.transform import compute_portfolio_summary, load_business_rules, load_loans, load_payments
    from src.legacy.validate_portfolio import load_validation_rules, validate_portfolio
...
def _rerun_payment_events(manifest: dict, workspace_dir: Path, repair_result: dict) -> tuple[dict, dict]:
    from src.legacy.transform import load_business_rules, load_loans, load_payment_events
    from src.legacy.validate_portfolio import load_validation_rules, validate_portfolio_from_payment_events
...
def _rerun_loan_payment_join(manifest: dict, workspace_dir: Path, repair_result: dict) -> tuple[dict, dict]:
    from src.legacy.transform import load_business_rules, load_loans, load_payments
    from src.legacy.validate_portfolio import load_validation_rules, validate_portfolio_with_join_profile
