...
**3b. Guaranteed auto-heal (scripted model):**
```
oc set env deployment/mcp-data-ops -n data-agent USE_SCRIPTED_MODEL=true
```
Call `create_candidate_repair` then `verify_candidate_repair` (RUNBOOK.md step 11's client
snippet) -> `VERIFIED_PENDING_PR` with a real diff, real before/after metrics, real passing
tests. Click **Accept** in the console's Review package card -> real `git apply` + real
pipeline rerun -> `loan_portfolio` comes back `PASS` on real cluster data.
